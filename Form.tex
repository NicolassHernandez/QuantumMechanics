...
\begin{table}[h!]
    ...
    \begin{tabular}{l|l}
        ...
        $[A,B]^\dagger=[B^\dagger,A^\dagger]$&$[F(A),A]=0$\\
        $[AB,CD]=A[B,C]D+AC[B,D]+[A,C]DB+C[A,D]B$&$[A,B]=0\Longrightarrow[F(A),B]=[A,F(B)]=[F(A),F(B)]=0$\\
        $[A,[B,C]]+[B,[C,A]]+[C,[A,B]]=0$&$[A,[A,B]]=[B,[A,B]]=0\Longrightarrow[A,F(B)]=[A,B]\frac{dF(B)}{dB}$\\
        $e^{A}e^{B}=e^{A+B}e^{\frac{1}{2}[A,B]}\;([A,[A,B]]=[B,[A,B]]=0)$&$[A,[A,B]]=[B,[A,B]]=0\Longrightarrow[F(A),B]=[A,B]\frac{dF(A)}{dA}$\\
